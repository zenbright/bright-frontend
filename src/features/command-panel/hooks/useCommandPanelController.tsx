...
export const useCommandPanelController = () => {
    ...
    const [nestedCommands, setNestedCommands] = React.useState<any[] | null>(
        null
    );
    ...
    const currentDataView = useSelector(
        (state: any) => state.dataViewMode.current
    );
    const [language, setLanguage] = React.useState(
        localStorage.getItem('devLanguage') || 'EN'
    );
    const [highlightModeEnabled, setHighlightModeEnabled] = React.useState(
        localStorage.getItem('highlightMode') === 'true'
    );
    ...
    React.useEffect(() => {
        ...
        window.addEventListener(
            ...
            handleHighlightModeChange as EventListener
        );
        return () =>
            window.removeEventListener(
                'highlightModeChanged',
    ...
    const toggleHighlightMode = () => {
        setHighlightModeEnabled(prev => {
            ...
            window.dispatchEvent(
                new CustomEvent('highlightModeChanged', { detail: newMode })
            );
            return newMode;
        });
    };
    ...
    const getCurrentSelection = (title: string) => {
        ...
        if (title === 'Toggle Highlight Mode')
            return highlightModeEnabled ? 'ON' : 'OFF';
        return null;
    };
    ...
    const displayCommands =
        !showAll && !nestedCommands ? flattenedCommands.slice(0, 7) : null;
